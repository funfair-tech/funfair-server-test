...
public sealed class TestAssemblyCodeGenerator : IIncrementalGenerator
{
    ...
    private static void GenerateVersionInformation(SourceProductionContext sourceProductionContext, (NamespaceError Left, AnalyzerConfigOptionsProvider Right) item)
    {
        ...
        GenerateAssemblySettings(sourceProductionContext: sourceProductionContext, analyzerConfigOptionsProvider: item.Right);
        GenerateEntryPoint(sourceProductionContext: sourceProductionContext, namespaceInfo: item.Left.NamespaceInfo.Value, analyzerConfigOptionsProvider: item.Right);
    }

    private static void GenerateAssemblySettings(in SourceProductionContext sourceProductionContext , AnalyzerConfigOptionsProvider analyzerConfigOptionsProvider)
    {
    ...
    private static void GenerateEntryPoint(in SourceProductionContext sourceProductionContext, in NamespaceGeneration namespaceInfo, AnalyzerConfigOptionsProvider analyzerConfigOptionsProvider)
    {
    ...
    {
        return analyzerConfigOptionsProvider.GlobalOptions.TryGetValue(key: "build_property.xunitautogeneratedentrypoint", out string? value) && StringComparer.OrdinalIgnoreCase.Equals(value, "false");
    }
