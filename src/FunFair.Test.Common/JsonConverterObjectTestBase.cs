    ...
    where TObject : class
{
    ...
    public void ShouldNotDeserialize()
    {
        ...
        JsonException exception = Assert.Throws<JsonException>(testCode: () =>
            JsonSerializer.Deserialize<Model>(json: doc, options: this._options)
        );
    ...
    [SuppressMessage(
        ...
        Justification = "Not a [MemberData] provider itself - a shared helper closed leaf classes call via "
            + "JsonConverterObjectTestBase<TConverter, TObject>.BuildDispatcherCases<TSelf>(), avoiding a "
            + "hand-copied case table per leaf"
    )]
    public static (string Name, Action<TSelf> Action)[] BuildDispatcherCases<TSelf>()
        where TSelf : JsonConverterObjectTestBase<TConverter, TObject> =>
